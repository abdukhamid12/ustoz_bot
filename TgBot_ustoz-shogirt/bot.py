...
#sherik"""
  await message.answer(context)
  await message.answer(f"Barcha ma'lumotlar to'g'rimi?", reply_markup=menu1)
  await state.finish()

@dp.message_handler(text='Yo`q')
async def not_handler(message: types.Message):
  await message.answer("Qabul qilinmadi ❌")
  await start_bot(message)

@dp.message_handler(text='Ha')
async def ha_handler(message: types.Message):
